...
class SierraEcgFile:
    ...
    signal_characteristic: Dict[str, str] = {}
...
def read_file(filename: str, include_repbeats: bool = False) -> SierraEcgFile:
    ...
    clinical_data = assert_clinical_data(root)
    signal_characteristic = assert_signal_characteristic(root)
    ...
    '''
    #print('lunghezza lead iii', len(lead_iii))
    for i in range(len(lead_iii)):
        lead_iii[i] = lead_ii[i] - lead_i[i]
    
    #print('lunghezza lead avr', len(lead_avr))
    for i in range(len(lead_avr)):
        lead_avr[i] = -floor((lead_i[i] + lead_ii[i]) / 2)

    #print('lunghezza lead avl', len(lead_avl))
    for i in range(len(lead_avl)):
        lead_avl[i] = floor((lead_i[i] - lead_iii[i]) / 2)

    #print('lunghezza lead avf', len(lead_avf))
    for i in range(len(lead_avf)):
        lead_avf[i] = floor((lead_ii[i] + lead_iii[i]) / 2)
    
    sierra_ecg_file = SierraEcgFile()
    sierra_ecg_file.doc_type = doc_type
    sierra_ecg_file.doc_ver = doc_ver
    sierra_ecg_file.leads = leads
    sierra_ecg_file.clinical_data = clinical_data
    sierra_ecg_file.signal_characteristic = signal_characteristic
...
def assert_clinical_data(elt: Document) -> Dict[str, str]:
    
    clinical_data_dict = {'name': get_text(get_node(get_node(get_node(get_node(elt, "patient"), "generalpatientdata"), 'name'), 'firstname')),
                          'surname': get_text(get_node(get_node(get_node(get_node(elt, "patient"), "generalpatientdata"), 'name'), 'lastname')),
                          'patient_ID': get_text(get_node(get_node(get_node(elt, "patient"), "generalpatientdata"), 'patientid')),
                          'date_of_birth': get_text(get_node(get_node(get_node(get_node(elt, "patient"), "generalpatientdata"), 'age'), 'dateofbirth')),
                          'sex': get_text(get_node(get_node(get_node(elt, "patient"), "generalpatientdata"), 'sex' ))}
    return clinical_data_dict

def assert_signal_characteristic(elt: Document) -> Dict[str, str]:
     
    signal_characteristic_dict = { 'sampling_rate': get_text(get_node(get_node(get_node(elt, "dataacquisition"), 'signalcharacteristics'), 'samplingrate')),
                                   'resolution': get_text(get_node(get_node(get_node(elt, "dataacquisition"), 'signalcharacteristics'), 'resolution')),
                                   'bits_per_sample': get_text(get_node(get_node(get_node(elt, "dataacquisition"), 'signalcharacteristics'), 'bitspersample')),
                                   'number_of_leads': get_text(get_node(get_node(get_node(elt, "dataacquisition"), 'signalcharacteristics'), 'numberchannelsvalid')),
                                   'hipass': get_text(get_node(get_node(get_node(elt, "dataacquisition"), 'signalcharacteristics'), 'hipass')),
                                   'lowpass': get_text(get_node(get_node(get_node(elt, "dataacquisition"), 'signalcharacteristics'), 'lowpass')),
                                   'acsetting': get_text(get_node(get_node(get_node(elt, "dataacquisition"), 'signalcharacteristics'), 'acsetting')),
                                   'durationperchannel': get_attr(get_node(get_node(elt, "waveforms"), 'parsedwaveforms'), 'durationperchannel')}
    
    return signal_characteristic_dict
